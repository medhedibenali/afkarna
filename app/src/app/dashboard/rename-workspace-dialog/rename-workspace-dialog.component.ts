  ...
  onSubmit() {
    if (this.renameForm.dirty && this.renameForm.valid) {
      ...
      if (this.item.workspace) {
        this.workspaceService.updateWorkspaceName(this.item.workspace.id, this.item.id, newName).subscribe({
          next: () => {
        ...
        this.workspaceItemService.updateItemName(this.item.id, newName).subscribe({
          next: () => {
            this.dialogRef.close(true);
          },
          error: (error) => {
